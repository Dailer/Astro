...
instruments=c('BTCUSDT','ATOMUSDT','NEARUSDT','ALGOUSDT','CRVUSDT','AAVEUSDT', 
              ...
              'SOLUSDT','SUSHIUSDT','TRXUSDT','UNIUSDT','XLMUSDT','XRPUSDT', 
              'YFIUSDT','COMPUSDT','PEPEUSDT','GRTUSDT','DASHUSDT', 
              'ETCUSDT','API3USDT','SKLUSDT','IOTXUSDT','EOSUSDT','NEOUSDT', 
              'ZECUSDT','BNBUSDT','XTZUSDT','BICOUSDT','BNTUSDT','OGNUSDT', 
              'ALICEUSDT','HBARUSDT','THETAUSDT','1INCHUSDT','ANKRUSDT', 
              'OXTUSDT','RENUSDT','STORJUSDT','ZRXUSDT','CELOUSDT','AMPUSDT', 
              'BANDUSDT','CTSIUSDT','ENSUSDT','UMAUSDT','LUNCUSDT','FTTUSDT', 
              'SRMUSDT','LPTUSDT','COTIUSDT','JASMYUSDT','HNTUSDT','KNCUSDT', 
              'METISUSDT','PYRUSDT','STXUSDT','STRKUSDT','SEIUSDT','INJUSDT', 
              'GHSTUSDT','KSMUSDT','OPUSDT','TIAUSDT','JTOUSDT','RPLUSDT', 
              'CROUSDT','FLTUSDT','FLRUSDT','ONDOUSDT','ORCAUSDT','OSMOUSDT', 
              'IOTAUSDT','FETUSDT','FILUSDT','IMXUSDT','RONINUSDT', 
              'AXLUSDT','SUIUSDT','LUNA2USDT','SGBUSDT')

bn_names=c('COMPUSDT','PEPEUSDT','GRTUSDT','IMXUSDT','RONINUSDT',
           'AXLUSDT','SUIUSDT','LUNA2USDT','SGBUSDT')
...
ui <- fluidPage(
  fluidRow(column(4, selectInput("source", "Source",
                              ...
                              "binance", width='60%')),
           column(4, offset=0,
                  selectInput("symb", "Symbol", instruments, width='50%')),
           column(4, offset=0, 
                  numericInput('ncand', 'Candles', 100, 60, 940, width='30%'))),
  radioButtons('temp', NULL, c('1m'='1m','5m'='5m','15m'='15m','30m'='30m',
                               '1h'='1h','2h'='2h','3h'='3h','4h'='4h',
                               '5h'='5h','6h'='6h','8h'='8h','10h'='10h',
                               '12h'='12h','18h'='18h','1d'='1d','1w'='1w'), 
...
server <- function(input, output, session){
  ...
  binance_klines2=function(symbol, interval=c('10m','20m','25m','45m','3h',
                                              '5h','7h','9h','10h','18h'), 
                           limit){
    btf=switch(interval, '10m'='5m', '20m'='5m', '25m'='5m', '45m'='15m', 
               '3h'='1h', '5h'='1h', '7h'='1h', '9h'='1h', '10h'='2h', '18h'='6h')
  ...
  get_quote_data=function(symbol, interval='1m', range='1d'){
    ...
    #                 4h, 5h, 6h, 8h, 10h, 12h, 1d
    require(rvest)
    require(jsonlite)
    require(data.table)
    new_int=c('10m','2h','3h','4h','5h','6h','8h','10h','12h','18h','7d')
    intervs=c('1m','2m','5m','15m','30m','1h','1d', new_int)
  ...
  data <- reactive({
    ...
    if(src()=='binance'){
      ...
      ntfs=c('10m','20m','25m','45m','3h','5h','7h','9h','10h','18h')
      if(!tf %in% ntfs){
  ...
  output$graph <- renderPlotly({
    x=data()
    ic=list(line=list(color='#26A69A'), fillcolor='#26A69A')
    dc=list(line=list(color='#EF5350'), fillcolor='#EF5350')
    tit=paste(x$symbol[1], x$tf[1], x$source[1], sep=' · ')
    ...
    fig1=plot_ly(data=x, x=~open_time, type="candlestick", open=~open, close=~close,
                 high=~high, low=~low, increasing=ic, decreasing=dc, name='') %>% 
      # add_lines(x=~open_time, y=~EMA10, line=list(color='#1C3D7E', width=2), 
      #           name='EMA 10',
      #           hoverinfo="none", inherit=F, showlegend=FALSE, hovertext='') %>% 
      add_lines(x=~open_time, y=~EMA55, line=list(color='#DC2663', width=2), 
                name='EMA 55', hoverinfo="none", inherit=F, showlegend=FALSE,
                hovertext='') %>%
